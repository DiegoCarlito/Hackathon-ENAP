...
def coleta_dados(session, component_id, viewstate, base_url, debug=False):
    ...
    """
    print(f"[{time.strftime('%H:%M:%S')}] Acessando detalhes do componente ID: {component_id}...")
    ...
    try:
        ...
        print(f"[{time.strftime('%H:%M:%S')}] Enviando POST para obter detalhes do componente...")
        ...
        if debug:
            with open(f"componente_{component_id}_response.html", "w", encoding="utf-8") as f:
                f.write(response.text)
        
        if response.status_code != 200:
            print(f"[{time.strftime('%H:%M:%S')}] ERRO ao acessar detalhes do componente: Status {response.status_code}")
            return {}
        ...
        if not details_soup or not details_soup.find_all():
            print(f"[{time.strftime('%H:%M:%S')}] ERRO: Não foi possível carregar o HTML da página de detalhes do componente.")
            return {}
            ...
            for row in rows:
                ...
                if th_element and td_element:
                    ...
                    if "Ementa/Descrição" in label:
                        ementa = value
                        print(f"[{time.strftime('%H:%M:%S')}] Ementa encontrada: {ementa[:50]}...")
        
        # Se não encontrou a tabela principal, tentar métodos alternativos
        if not details:
            print(f"[{time.strftime('%H:%M:%S')}] Tentando métodos alternativos para encontrar dados...")
        ...
        print(f"[{time.strftime('%H:%M:%S')}] Detalhes do componente {component_id} extraídos com sucesso!")
        return component_details
        
    except Exception as e:
        print(f"[{time.strftime('%H:%M:%S')}] ERRO ao processar detalhes do componente: {str(e)}")
        import traceback
        traceback.print_exc()
        return {}
...
def scrape_unb_classes(debug=False):
    ...
    try:
        ...
        if response.status_code != 200:
            print(f"[{time.strftime('%H:%M:%S')}] ERRO: Status da resposta: {response.status_code}")
            return None
    ...
    print(f"[{time.strftime('%H:%M:%S')}] Parâmetros da busca: GRADUAÇÃO, CAMPUS UNB GAMA, 2025-1")
    ...
    try:
        ...
        if search_response.status_code != 200:
            print(f"[{time.strftime('%H:%M:%S')}] ERRO: Status da resposta POST: {search_response.status_code}")
            return None
    except requests.exceptions.RequestException as e:
    ...
    if "Nenhuma turma encontrada" in search_response.text:
        print(f"[{time.strftime('%H:%M:%S')}] AVISO: Nenhuma turma encontrada com os critérios informados.")
        return None
    ...
    if not tables:
        print(f"[{time.strftime('%H:%M:%S')}] ERRO: Tabela de resultados não encontrada.")
        return None
    
    print(f"[{time.strftime('%H:%M:%S')}] Encontradas {len(tables)} tabelas de resultados.")
    ...
    for table_counter, table in enumerate(tables, 1):
        ...
        rows = table.find_all("tr")
        print(f"[{time.strftime('%H:%M:%S')}] Processando tabela {table_counter} com {len(rows)} linhas.")
            ...
            if "agrupador" in row.get("class", []):
                ...
                if link:
                    ...
                    if id_match:
                        ...
                        print(f"[{time.strftime('%H:%M:%S')}] Encontrado componente: {current_component_name} (ID: {current_component_id})")
                        
                        # Adicionar um pequeno delay para não sobrecarregar o servidor
                        time.sleep(1)
                        
                        # Obter detalhes do componente
                        current_component_details = coleta_dados(session, current_component_id, viewstate, base_url, debug)
                    else:
                        print(f"[{time.strftime('%H:%M:%S')}] AVISO: Não foi possível extrair o ID do componente do onclick: {onclick}")
                        current_component_id = None
                        current_component_details = {}
            ...
            # Para as linhas de dados (não agrupador e não cabeçalho)
                ...
                if len(cols) >= 5:  # Verificar se a linha tem colunas suficientes
                    try:
                        ...
                        print(f"[{time.strftime('%H:%M:%S')}] ERRO ao processar uma linha: {str(e)}")
    
    if not turmas_data:
        print(f"[{time.strftime('%H:%M:%S')}] ERRO: Nenhum dado encontrado nas tabelas.")
        return None
    
    # Criar DataFrame com os resultados
    df_turmas = pd.DataFrame(turmas_data)
    
    print(f"[{time.strftime('%H:%M:%S')}] Extração concluída. Foram encontradas {len(df_turmas)} turmas.")
...
if __name__ == "__main__":
    try:
        ...
        if len(sys.argv) > 1 and sys.argv[1].lower() == '--debug':
            debug_mode = True
            print(f"[{time.strftime('%H:%M:%S')}] Modo de depuração ativado. Os arquivos HTML serão salvos.")
            
        resultado = scrape_unb_classes(debug=debug_mode)
        if resultado is not None:
            print(f"[{time.strftime('%H:%M:%S')}] Primeiras linhas dos dados extraídos:")
            print(resultado.head())
        else:
            print(f"[{time.strftime('%H:%M:%S')}] Nenhum resultado foi retornado.")
    except Exception as e:
        print(f"[{time.strftime('%H:%M:%S')}] ERRO CRÍTICO durante o scraping: {str(e)}")
        import traceback
        traceback.print_exc()
